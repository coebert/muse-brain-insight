/**
 * Force a COEBIS refit for one lineage prefix, bypassing the "unchanged data"
 * skip so an existing model can be re-graded on demand. Uses the app pipeline
 * (validation gate, refitLineage, promotion rule) and persists a version row
 * exactly like runRefitForUser does.
 */
import { readFileSync } from "node:fs";
import { createClient } from "@supabase/supabase-js";
import { loadTrainingMatrix } from "@/lib/eeg/coebis-training.server";
import { selectValidatedPoints, refitLineage } from "@/lib/eeg/coebis-refit";
import { modelFromRow } from "@/lib/eeg/coebis-refit.server";

const prefix = process.argv[2] ?? "dose-i";
...
for (const [key, points] of byLineage) {
  const result = refitLineage(key, points, incumbents.get(key) ?? null);
  console.log(
    JSON.stringify(
      {
        lineageKey: key,
  ...
  const { error } = await admin.from("coebis_model_versions").upsert(
    {
      ...
      training: { n: result.n, cases: result.cases, folds: result.folds, lineageKey: key },
