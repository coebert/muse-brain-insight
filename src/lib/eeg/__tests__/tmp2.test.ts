...
for(let i=0;i<n;i++){const t=i/FS;const v=20*Math.sin(2*Math.PI*10*t)+5*Math.sin(2*Math.PI*3*t)+2*(Math.random()-0.5);rows.push(`${t.toFixed(4)},${v.toFixed(3)},${(v*0.9).toFixed(3)}`);}return rows.join("\n");}
